...
namespace Ubik.Web
{
    public static class Pagination
    {
         ...
         object htmlAttributes = null)
        {
            /*validation*/
            if (metaData == null)
                throw new ArgumentNullException("metaData", "A navigation collection is mandatory.");
            if (!metaData.Any()) return MvcHtmlString.Empty;
            if (settings == null) settings = new PaginationInfo();

            /*validation*/
            return MvcHtmlString.Create(GetHtmlPager(helper, metaData, generatePageUrl, routedValues, settings, htmlAttributes));
        }
            ...
            PaginationInfo settings)
        {
            ...
            {
                InnerHtml = settings.NextDisplay
            };
            ...
            PaginationInfo settings)
        {
            ...
            {
                InnerHtml = settings.LastDisplay
            };
        ...
        private static TagBuilder PageCountAndLocationText<T>(IPagedResult<T> metadata, PaginationInfo settings)
        {
            ...
            text.SetInnerText(string.Format(settings.PageCountAndLocationFormat, metadata.PageNumber, metadata.TotalPages));
            text.MergeAttribute("href",
                string.Empty, true);
            item.InnerHtml = text.ToString();
            return item;
        }
        ...
        private static TagBuilder ItemSliceAndTotalText<T>(IPagedResult<T> metadata, PaginationInfo settings)
        {
            ...
            var text = new TagBuilder("a");
            text.SetInnerText(string.Format(settings.ItemSliceAndTotalFormat, FirstItemOnPage, LastItemOnPage, metadata.TotalRecords));
            text.MergeAttribute("href",
                string.Empty, true);
            item.InnerHtml = text.ToString();
            return item;
        ...
        {
            var targetPageNumber = metaData.PageNumber - ((settings.MaximumPageNumbersToDisplay.HasValue) ? settings.MaximumPageNumbersToDisplay.Value : 10);
            if (targetPageNumber < 1) targetPageNumber = 1;

            var item = new TagBuilder("li");
            var a = new TagBuilder("a")
            {
                InnerHtml = settings.Ellipses
            };
        ...
        private static TagBuilder Page<T>(int i, IPagedResult<T> metaData, Func<int, string> generatePageUrl, NameValueCollection routedValues, PaginationInfo settings)
        {
            ...
            page.SetInnerText(string.Format(settings.PageDisplayFormat, i));
        ...
        private static TagBuilder Previous<T>(IPagedResult<T> metaData, Func<int, string> generatePageUrl, NameValueCollection routedValues, PaginationInfo settings)
        {
            ...
            {
                InnerHtml = settings.PreviousDisplay
            };
            ...
            item.InnerHtml = previous.ToString();
            return item;
        }
        ...
        private static TagBuilder First<T>(IPagedResult<T> metaData, Func<int, string> generatePageUrl, NameValueCollection routedValues, PaginationInfo settings)
        {
            ...
            {
                InnerHtml = settings.FirstDisplay
            };
            ...
            item.InnerHtml = first.ToString();
            return item;
        }

        #endregion Private Zurb Pagination

        #endregion Zurb Pagination
